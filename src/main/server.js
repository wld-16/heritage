...
app.get('/api/person/list', (req, res) => {
	client.query("select * from person").then(data => {
		res.send(data.rows)
	})
})
...
app.get('/api/animal/list', (req, res) => {
	client.query("select * from animal").then(data => {
		res.send(data.rows)
	})
})
...
app.get('/api/species/list', (req, res) => {
	client.query("select * from species").then(data => {
		res.send(data.rows)
	})
})
...
app.get('/api/race/list', (req, res) => {
	client.query("select * from race").then(data => {
		res.send(data.rows)
	})
})
...
app.get('/api/relationship-label/list', (req, res) => {
	client.query("select * from relationship").then(data => {
		res.send(data.rows)
	})
})
...
app.post('/api/relationship-label/create', (req, res) => {
	...
	var oppositeId = 0;
	var mainId = 0;
	var mainValuesEntry = [req.body.mainlabel, oppositeId]
	client.query(insertQuery, mainValuesEntry).then(res => {
		console.log("Main:" + res.rows[0])
		mainId = res.rows[0].id
	})
	var oppositeValuesEntry = [req.body.oppositeLabel, mainId]
	client.query(insertQuery, oppositeValuesEntry).then(res => {
		console.log("Opposite: " + res.rows[0])
		oppositeId = res.rows[0].id
	})
	const updateQuery = "UPDATE Relationship SET opposite_id = $1 where id = $2";
	client.query(updateQuery, [oppositeId, mainId]).then(res => {
		console.log("Update Main: " + res.rows[0])
	})
})
